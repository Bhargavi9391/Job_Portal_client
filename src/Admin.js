...
export default function Admin() {
  ...
  const handleDelete = (index) => {
    ...
    // Remove job from homePostedJobs localStorage if present
  ...
  const handlePostJob = async (job) => {
    ...
    try {
      const response = await fetch("https://job-portal-server-1-nt8w.onrender.com/api/jobs", {
      ...
      const jobWithTimestamp = { ...job, postedTime: Date.now(), _id: result._id };
  ...
  return (
    <div className="admin-container">
      ...
      <h2 className="submitted-title">Submitted Job Postings</h2>
      <div className="submitted-jobs">
        {submittedData.length === 0 && <p>No jobs submitted yet.</p>}
        {submittedData.map((job, index) => (
          <div key={index} className="job-card">
            ...
            <p>
              <strong>Expected Year:</strong> {job.expectedYear}
            </p>
            <p>
